...
class Resume(models.Model):
    ...
    position = models.CharField(blank=True, null=True, verbose_name='申请职位')
    experience = models.TextField(blank=True, null=True, verbose_name='学习或工作经历')
    photo = models.ImageField(
        upload_to='contact/recruit/%Y_%m_%d', 
        verbose_name='个人照片'
    )
...
def post_save_resume(sender, instance, **kwargs):
    ...
    if instance.__original_status == 1 and instance.status == 2:
        ...
        template_path = os.path.join(settings.BASE_DIR, 'media', 'recruit.docx')
        ...
        if instance.photo and hasattr(instance.photo, 'path') and os.path.exists(instance.photo.path):
        ...
        output_dir = os.path.join(settings.BASE_DIR, 'media', 'contact', 'recruit')
        os.makedirs(output_dir, exist_ok=True)
        
        filename = os.path.join(output_dir, f'{instance.name}_{instance.id}.docx')
